...
try {
    ...
    // Debug: Log current working directory and appstate file path
    console.log('🔍 Debug - Current working directory:', process.cwd());
    console.log('🔍 Debug - AppState file path:', appStateFile);
    console.log('🔍 Debug - Main path:', global.client.mainPath);
    
    // Kiểm tra environment variable trước
    let appState;
    if (process.env.FACEBOOK_APPSTATE) {
        try {
            appState = JSON.parse(process.env.FACEBOOK_APPSTATE);
            logger.loader("Đã tìm thấy appstate từ environment variable", "LOGIN");
        } catch (e) {
            logger.loader("Lỗi parse appstate từ environment variable, thử đọc từ file", "LOGIN");
    ...
    // Nếu không có environment variable và file không tồn tại, thử fetch từ API
    if (process.env.FACEBOOK_APPSTATE) {
        try {
            appState = JSON.parse(process.env.FACEBOOK_APPSTATE);
            logger.loader("Đã tìm thấy appstate từ environment variable", "LOGIN");
        } catch (e) {
            return logger.loader("Lỗi parse appstate từ environment variable và không tìm thấy file appstate.json", "error");
        }
    } else {
...
async function fetchAppState() {
  ...
  try {
    ...
    logger(`Đã tìm thấy appstate`, "LOGIN");
      ...
      // Ghi ra file
      fs.writeFileSync(path.join(process.cwd(), 'appstate.json'), JSON.stringify(appState, null, 2), 'utf8');
